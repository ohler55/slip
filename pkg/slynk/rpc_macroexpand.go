...
func init() {
	...
	RegisterHandler("slynk:swank-macroexpand-1", handleMacroexpand1)
	RegisterHandler("slynk:swank-macroexpand", handleMacroexpand)
	RegisterHandler("slynk:swank-macroexpand-all", handleMacroexpandAll)

	// Shorthand aliases
	RegisterHandler("slynk:swank-expand-1", handleMacroexpand1)
	RegisterHandler("slynk:swank-expand", handleMacroexpand)
}
...
func macroexpand1(c *Connection, form slip.Object) slip.Object {
	...
	args := list[1:]
	ch := c.DefaultChannel()
	expanded := callMacro(ch.Scope(), fi, args)
	if expanded == nil {
		return form
	}
