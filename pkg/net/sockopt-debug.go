...
func init() {
	slip.Define(
		func(args slip.List) slip.Object {
			f := SockoptDebug{Function: slip.Function{Name: "sockopt-debug", Args: args}}
			f.Self = &f
			return &f
		},
		&slip.FuncDoc{
			...
			Text:   `__sockopt-debug__ returns the :keep-alive on the _socket_ instance.`,
			Examples: []string{
				`(sockopt-debug (make-instance 'socket :socket 5)) => nil`,
			},
		}, &Pkg)
}
