//go:build windows

// Copyright (c) 2024, Peter Ohler, All rights reserved.

package gi

import (
	"fmt"

	"github.com/ohler55/slip"
)

func init() {
	slip.Define(
		func(args slip.List) slip.Object {
			f := SendSignal{Function: slip.Function{Name: "send-signal", Args: args}}
			f.Self = &f
			return &f
		},
		&slip.FuncDoc{
			Name: "send-signal",
			Args: []*slip.DocArg{
				{
					Name: "pid",
					Type: "fixnum",
					Text: "process to send the signal to.",
				},
				{
					Name: "signal",
					Type: "fixnum",
					Text: "signal to send.",
				},
			},
			Text: `__send-signal__ send _signal_ to _pid_. (Not fully supported on Windows)`,
			Examples: []string{
				`(send-signal 12345 sigint) => nil`,
			},
		}, &Pkg)
}

// SendSignal represents the send-signal function.
type SendSignal struct {
	slip.Function
}

// Call the function with the arguments provided.
// On Windows, syscall.Kill is not available. This is a stub that returns an error.
func (f *SendSignal) Call(s *slip.Scope, args slip.List, depth int) slip.Object {
	slip.CheckArgCount(s, depth, f, args, 2, 2)
	pid, ok := args[0].(slip.Fixnum)
	if !ok {
		slip.TypePanic(s, depth, "pid", args[0], "fixnum")
	}
	var sig slip.Fixnum
	if sig, ok = args[1].(slip.Fixnum); !ok {
		slip.TypePanic(s, depth, "signal", args[1], "fixnum")
	}
	// On Windows, sending signals like on Unix is not directly supported.
	// We'll panic with an error message indicating this limitation.
	panic(fmt.Errorf("send-signal is not supported on Windows (pid=%d, signal=%d)", pid, sig))
}
