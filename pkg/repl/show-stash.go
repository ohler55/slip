...
func init() {
	slip.Define(
		func(args slip.List) slip.Object {
			f := ShowStash{Function: slip.Function{Name: "show-stash", Args: args}}
			f.Self = &f
			return &f
		},
		&slip.FuncDoc{
			...
			Args: []*slip.DocArg{
				{Name: "&optional"},
				{
					...
					Text: "Destination to write stash to.",
				},
				{Name: "&key"},
				{
					Name: "annotate",
					Type: "boolean",
					Text: "If true annotate the output with the stash index (1 based).",
				},
				{
					Name: "tight",
					Type: "boolean",
				...
				},
				{
					Name: "start",
					Type: "fixnum",
					Text: "The start index of the stash to show.",
				},
				{
					Name: "end",
					Type: "fixnum",
					Text: "The end index of the stash to show.",
				},
			},
			Return: "nil|string",
			Text: `__show-stash__ writes stash to _destination_. If _destination_ is true,
the default value stash is written to _*standard-output*_. If _nil_ stash is returned as
a _string_. If an _output-stream_ stash is written to the stream.`,
		}, &Pkg)
}
...
func (f *ShowStash) Call(s *slip.Scope, args slip.List, depth int) slip.Object {
	return showStashCall(f, &TheStash, s, args)
}
