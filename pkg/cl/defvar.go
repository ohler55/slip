...
func (f *Defvar) Call(s *slip.Scope, args slip.List, depth int) (result slip.Object) {
	...
	if !ok {
		slip.TypePanic(s, depth, "name argument to defvar", args[0], "symbol")
	}
	name := strings.ToLower(string(sym))
	pkg, vname, private := slip.UnpackName(name)
	if pkg == nil {
	...
	if 1 < len(args) {
		...
		if 2 < len(args) {
			...
			if doc, ok = args[2].(slip.String); !ok {
				slip.TypePanic(s, depth, "documentation argument to defvar", args[2], "string")
			}
		}
	}
