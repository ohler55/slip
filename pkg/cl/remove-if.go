...
func init() {
	slip.Define(
		func(args slip.List) slip.Object {
			f := RemoveIf{DeleteIf{Function: slip.Function{Name: "remove-if", Args: args}}}
			f.Self = &f
			return &f
		},
		&slip.FuncDoc{
...
in the _sequence_ to return a key for comparison.`,
			...
			Return: "object",
			Text:   `__remove-if__ returns a sequence with the matching elements removed.`,
			Examples: []string{
				"(remove-if 'numberp '((y . 1) (1 . 2) (z . 3)) :key 'car) => ((y . 1) (z. 3))",
			},
		}, &slip.CLPkg)
}
