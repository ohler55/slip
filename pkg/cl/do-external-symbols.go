...
func init() {
	slip.Define(
		...
		&slip.FuncDoc{
			...
			Return: "list",
			Text: `__do-external-symbols__ expects the _args_ to be a list of a symbol which is not evaluated
and an optional package which is evaluated. An optional form can also be included in the _args_ that
is evaluated and the result returned. For each exported symbol in the package the _forms_ are called.`,
			Examples: []string{
				"(let ((lst ()))",
				" (do-external-symbols (s *watch* lst) (add lst s))) => (watch-server watch-client ...)",
			},
		}, &slip.CLPkg)
}
