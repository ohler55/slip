...
func init() {
	slip.Define(
		func(args slip.List) slip.Object {
			f := Nsublis{Function: slip.Function{Name: "nsublis", Args: args}}
			f.Self = &f
			return &f
		},
		&slip.FuncDoc{
...
func (f *Nsublis) Call(s *slip.Scope, args slip.List, depth int) (result slip.Object) {
	...
	return replaceTree(s, tree, alist, kc, tc, depth+1)
}

func replaceTree(s *slip.Scope, tree slip.Object, subs slip.List, kc, tc slip.Caller, depth int) slip.Object {
	key := tree
	if kc != nil {
	...
	if list, ok := tree.(slip.List); ok {
		for i, e := range list {
			if tail, ok2 := e.(slip.Tail); ok2 {
				// Need a new tail since it is not a pointer.
				list[i] = slip.Tail{Value: replaceTree(s, tail.Value, subs, kc, tc, depth)}
			} else {
				list[i] = replaceTree(s, e, subs, kc, tc, depth)
			}
		}
	}
	return tree
