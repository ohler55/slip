...
func init() {
	slip.Define(
		func(args slip.List) slip.Object {
			f := NsubstituteIf{Function: slip.Function{Name: "nsubstitute-if", Args: args}}
			f.Self = &f
			return &f
		},
		&slip.FuncDoc{
...
in the _alist_ to return a key for comparison. The same function is also applied to _item_.`,
			...
			Return: "sequence",
			Text:   `__nsubstitute-if__ returns a copy _sequence_ with _old_ nsubstitute-ifd with _new_.`,
			Examples: []string{
				"(setq lst '(a b c)",
				"(nsubstitute-if 2 (lambda (v) (equal v 'b)) lst) => (a 2 c)",
				"lst => (a 2 c)",
			},
		}, &slip.CLPkg)
}
