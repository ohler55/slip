...
func init() {
	slip.Define(
		...
		&slip.FuncDoc{
			...
			Return: "object",
			Text: `__setq__ the value of the _symbol_ to _value_. Note that _symbol_ is not evaluated.
...
func (f *Setq) Call(s *slip.Scope, args slip.List, depth int) (result slip.Object) {
	if len(args)%2 != 0 {
		slip.ErrorPanic(s, depth, "setq expected symbol value pairs. Not %d arguments.", len(f.Args))
	}
	d2 := depth + 1
	last := len(args) - 1
	for i := 0; i < last; i++ {
