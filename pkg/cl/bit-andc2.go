...
func init() {
	slip.Define(
		func(args slip.List) slip.Object {
			f := BitAndc2{Function: slip.Function{Name: "bit-andc2", Args: args}}
			f.Self = &f
			return &f
		},
		&slip.FuncDoc{
			...
			Return: "bit-array",
			Text: `__bit-andc2__ returns an array with the same dimension as both _bit-array1_ and
_bit-array2_ that is a logical AND of _bit-array1_ and the complement of _bit-array2_. If _opt-arg_
is provided it is used as the result array.`,
			Examples: []string{
				"(bit-andc2 #*1010 #*1100) => #*0010",
			},
		}, &slip.CLPkg)
}
...
func (f *BitAndc2) Call(s *slip.Scope, args slip.List, depth int) (result slip.Object) {
	...
	case *slip.Array:
		...
		for i, v := range e1 {
			if v == slip.Bit(1) && e2[i] == slip.Bit(0) {
