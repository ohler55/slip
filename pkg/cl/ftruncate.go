// Copyright (c) 2022, Peter Ohler, All rights reserved.

package cl

import (
	"math/big"

	"github.com/ohler55/slip"
)

func init() {
	slip.Define(
		func(args slip.List) slip.Object {
			f := Ftruncate{Function: slip.Function{Name: "ftruncate", Args: args}}
			f.Self = &f
			return &f
		},
		&slip.FuncDoc{
			Name: "ftruncate",
			Args: []*slip.DocArg{
				{
					Name: "number",
					Type: "real",
					Text: `The number to take the quotient of and then truncateed
to the next integer towards positive infinity.`,
				},
				{Name: "&optional"},
				{
					Name: "divisor",
					Type: "real",
					Text: `The number to divide the _number_ by. The default is 1.`,
				},
			},
			Return: "float,real",
			Text: `__ftruncate__ returns the quotient of the _numbers_ truncateed
toward positive infinity as well as the remainder.`,
			Examples: []string{
				"(ftruncate 5.4) => 5.0, 0.4",
				"(ftruncate 3/2) => 1.0, 1/2",
				"(ftruncate 5 2) => 2.0, 11",
			},
		}, &slip.CLPkg)
}

// Ftruncate represents the ftruncate function.
type Ftruncate struct {
	slip.Function
}

// Call the function with the arguments provided.
func (f *Ftruncate) Call(s *slip.Scope, args slip.List, depth int) slip.Object {
	values := truncate(s, f, args, depth)
	switch tv := values[0].(type) {
	case slip.Fixnum:
		switch values[1].(type) {
		case slip.SingleFloat:
			values[0] = slip.SingleFloat(tv)
		default:
			values[0] = slip.DoubleFloat(tv)
		}
	case *slip.Bignum:
		if (*big.Int)(tv).IsInt64() {
			if _, ok := values[1].(*slip.LongFloat); ok {
				var z big.Float
				values[0] = (*slip.LongFloat)(z.SetInt((*big.Int)(tv)))
			} else {
				values[0] = slip.DoubleFloat((*big.Int)(tv).Int64())
			}
		} else {
			var z big.Float
			values[0] = (*slip.LongFloat)(z.SetInt((*big.Int)(tv)))
		}
	}
	return values
}
