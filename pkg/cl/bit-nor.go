...
func init() {
	slip.Define(
		func(args slip.List) slip.Object {
			f := BitNor{Function: slip.Function{Name: "bit-nor", Args: args}}
			f.Self = &f
			return &f
		},
		&slip.FuncDoc{
			...
			Return: "bit-array",
			Text: `__bit-nor__ returns an array with the same dimension as both _bit-array1_ and
_bit-array2_ that is a logical NOR (complement of an or) of _bit-array1_ and _bit-array2_. If _opt-arg_
is provided it is used as the result array.`,
			Examples: []string{
				"(bit-nor #*1010 #*1100) => #*0001",
			},
		}, &slip.CLPkg)
}
...
func (f *BitNor) Call(s *slip.Scope, args slip.List, depth int) (result slip.Object) {
	...
	case *slip.Array:
		...
		for i, v := range e1 {
			if v == slip.Bit(0) && e2[i] == slip.Bit(0) {
