...
func TestDefLambdaArgs(t *testing.T) {
	...
	tt.Equal(t, `(lambda (x y) "docs" (1+ 1))`, lam.LoadForm().String())
}
...
func TestLambdaCallWithScope(t *testing.T) {
	scope := slip.NewScope()
	_ = slip.ReadString(`(let ((y 3)) (setq call-it (lambda (x) (+ x y))))`, scope).Eval(scope, nil)
	(&sliptest.Function{
		Scope:  scope,
...
func TestLambdaCallInFunc(t *testing.T) {
	...
	_ = slip.ReadString(`
(defun quux (x)
  (apply (lambda (y) (+ x y)) '(5)))`, scope).Eval(scope, nil)
	(&sliptest.Function{
		Scope:  scope,
		Source: `(quux 7)`,
		Expect: "12",
