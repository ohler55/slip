...
func TestFindSymbolNew(t *testing.T) {
	...
	scope := slip.NewScope()
	_ = slip.ReadString("(defvar find-symbol-test-new 7)", scope).Eval(scope, nil)
	(&sliptest.Function{
		Source: `(find-symbol "find-symbol-test-new")`,
...
func TestFindSymbolExternal(t *testing.T) {
	...
	scope := slip.NewScope()
	_ = slip.ReadString(`(intern "find-symbol-test-external" 'keyword)`, scope).Eval(scope, nil)
	(&sliptest.Function{
		Source: `(find-symbol "find-symbol-test-external" "keyword")`,
...
func TestFindSymbolInternal(t *testing.T) {
	scope := slip.NewScope()
	_ = slip.ReadString(`(defun find-symbol-test-internal () 5)`, scope).Eval(scope, nil)
	(&sliptest.Function{
