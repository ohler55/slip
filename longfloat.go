...
digits and a maximum exponent of 308.`)
	mostPos, _, _ := big.ParseFloat(fmt.Sprintf("1.0e%d", big.MaxExp), 10, 10, big.ToNearestAway)
	mostNeg, _, _ := big.ParseFloat(fmt.Sprintf("-1.0e%d", big.MaxExp), 10, 10, big.ToNearestAway)
	...
	epsilon, _, _ := big.ParseFloat(fmt.Sprintf("1.0e%d", big.MaxPrec), 10, 10, big.ToNearestAway)
